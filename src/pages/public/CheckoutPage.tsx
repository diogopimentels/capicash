...
import { Check, Copy, Download, Loader2, Lock, Mail, QrCode, ShieldCheck, CheckCircle2, Zap, Image as ImageIcon } from "lucide-react"
...
const MOCK_PRODUCT = {
    id: "prod_123",
    title: "Pack de Presets Lightroom - Master Collection",
    description: "Transforme suas fotos com um clique. 50+ presets profissionais para desktop e mobile. Edição de nível profissional em segundos.",
...
export function CheckoutPage() {
    ...
    const handleCopyPix = () => {
        navigator.clipboard.writeText("00020126580014BR.GOV.BCB.PIX0136123e4567-e89b-12d3-a456-426614174000520400005303986540510.005802BR5913Joao Foto6008Brasilia62070503***6304E2CA")
        setCopied(true)
                ...
                <div className="w-full max-w-5xl bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20 grid grid-cols-2">
                    ...
                    <div className="p-12 bg-zinc-50/50 dark:bg-black/20 flex flex-col justify-between">
                        <div className="space-y-6">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold text-zinc-900 dark:text-white mb-2">{MOCK_PRODUCT.title}</h1>
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <span className="text-sm font-medium text-zinc-600 dark:text-zinc-300">Por {MOCK_PRODUCT.seller.name}</span>
                                    {MOCK_PRODUCT.seller.verified && <Badge variant="secondary" className="text-xs bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800">Verificado</Badge>}
                                </div>
                                <p className="text-zinc-500 dark:text-zinc-400 leading-relaxed text-sm">{MOCK_PRODUCT.description}</p>
                            </div>
                                ...
                                >
                                    ...
                                    <div className="space-y-4 pt-4">
                                        <div className="flex justify-between items-end pb-4 border-b border-zinc-200 dark:border-white/10">
                                            ...
                                            <span className="text-3xl font-bold text-zinc-900 dark:text-white">{formatPrice(MOCK_PRODUCT.price)}</span>
                                        </div>
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            <Input
                                                readOnly
                                                value="00020126580014BR.GOV.BCB.PIX0136123e4567-e89b-12d3-a456..."
                            ...
                            {step === "success" && (
                                ...
                                >
                                    ...
                                    <div className="bg-zinc-50 dark:bg-white/5 p-4 rounded-lg flex items-center gap-4 text-left border border-zinc-100 dark:border-white/10">
                                        ...
                                        <div>
                                            <p className="font-medium text-sm text-zinc-900 dark:text-zinc-100">{MOCK_PRODUCT.title}</p>
            ...
            <div className="md:hidden flex flex-col min-h-screen pb-24 relative z-10">
                ...
                <div className="relative">
                    ...
                    <div className="h-64 w-full bg-gradient-to-br from-emerald-400 to-cyan-500 relative">
                        ...
                        <div className="absolute bottom-6 left-5 right-5 z-[2] text-white">
                            <Badge className="bg-emerald-500 mb-2 border-0 hover:bg-emerald-600">{formatPrice(MOCK_PRODUCT.price)}</Badge>
                            <h1 className="text-xl font-bold leading-tight shadow-sm">{MOCK_PRODUCT.title}</h1>
                            <p className="text-xs text-zinc-300 mt-1 truncate">Por {MOCK_PRODUCT.seller.name}</p>
                        </div>
                    </div>
                </div>
                ...
                {step === "email" && (
                    <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-t border-zinc-200 dark:border-white/5 z-50 safe-area-bottom">
                        ...
                        >
                            {loading ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : null}
                            Pagar {formatPrice(MOCK_PRODUCT.price)}
                        </Button>
